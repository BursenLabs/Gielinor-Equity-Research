...
import { initUI, showDisclaimer } from "./ui";
...
function setStartupStatus(msg: string): void {
  if (startupStatus) {
...
(async () => {
  try {
    ...
    alt1Status?.insertAdjacentHTML(
      "beforeend",
      `<div style="color:var(--accent-red);margin-top:4px">Startup error \u2014 see console.</div>`
    );
  }
})();
