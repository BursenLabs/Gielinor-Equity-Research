# Gielinor Equity Research — Alt1 Toolkit Plugin

An intelligent **Grand Exchange market analyzer, portfolio tracker, and AI flipping advisor** for RuneScape 3, built as an [Alt1 Toolkit](https://runeapps.org/alt1) overlay plugin.
...
* [Alt1minimal](https://github.com/skillbert/alt1minimal/) - The official template by Skillbert used as the foundation for this project.
---
...
- All app state — settings, favourites, portfolio, flip history, and price cache — is stored exclusively in your browser's [**localStorage and IndexedDB**](src/services/cacheService.ts). Nothing is sent to any server.
- Any LLM API keys you provide are stored only in your browser and are [sent directly from your machine to the provider](src/services/llmProviders.ts) you choose. They are never relayed through a third-party server.
- The only outbound network requests are to public, read-only endpoints: the [daily baseline JSON](src/services/baselineService.ts) served from GitHub Pages, the [Weird Gloop API](src/services/weirdGloopService.ts) ([api.weirdgloop.org](https://api.weirdgloop.org/#/exchange)) for live RS3 GE prices, and the [RS Wiki](https://runescape.wiki) for [news headlines](src/services/newsService.ts). None of these requests include any personally identifiable information.
...
- **Smart badges** — trade velocity (Insta-Flip / Active / Slow), hype detection (volume spikes), price momentum warnings, and forecast estimates at a glance
...
- **Per-section sorting** — independent sort controls (Default, A–Z, Price ↓, Profit ↓, GP/hr ↓) on Top 20, Search Results, and Favourites
- **Dynamic filters** — volume, price, and confidence sliders with custom range controls
- **Full GE search** — search all ~7,000 tradeable items with on-demand price fetching
- **Instant startup** — pre-built daily baseline delivers ~7,000 items with prices, buy limits, and alch values in ~1–2 seconds (no API calls needed at startup)
- **Background market scan** — scan the full catalogue (~30 s for prices, ~13 min with deep history) with progress bar and cancel support; optional 90-day deep history for complete sparklines (available in Settings → Data when Advanced Mode is on)
...
- **RS3 news awareness** — recent game update headlines from the RS Wiki are automatically included in the advisor's context, helping it explain why items may be rising or dropping (toggleable)
...
- **Market snapshots** — captures 10-field market state at buy and sell time for side-by-side comparison
- **Mark as sold** — record actual sell price to calculate realised profit (post-tax)
- **Alch exit** — complete a flip via High Alchemy (no GE tax) with the 🔥 button
- **Edit flips** — patch buy price, quantity, or target/actual sell price on active or completed flips
- **Flip detail modal** — trade summary with buy-vs-sell snapshot comparison table and delta column
- **Buy-limit reset notifications** — toast + native notification when a flip’s 4-hour window expires (toggleable)
...
- **Capital allocation suggestions** — enter your cash stack and get optimised 8-slot GE spend plans (Balanced spread or Aggressive profit-maximising)
...
- **Cache database export / import** — serialise and restore the full IndexedDB price cache (prices + history)
- **Quick-add** — one-click add from any market card to the portfolio form
- **Disclaimer modal** — startup acknowledgement explaining data limitations and that this is not financial advice
...
- **Contrast auto-correction** — binary-search blend algorithm ensures WCAG AA compliance across all theme combinations
...
```
Daily Baseline (CI) → IndexedDB cache → deterministic filtering → LLM synthesis
                 ↑ fallback: Weird Gloop API → wiki enrichment
```

A GitHub Actions workflow generates a daily baseline (`data/baseline.json`) containing ~7,000 items with latest prices, buy limits, and high alch values. On startup, the app loads this baseline in ~1–2 seconds. If the baseline is unavailable, it falls back to fetching ~230 seed items from the Weird Gloop API.
...
Copyright (c) 2026 BursenLabs. Released under the [ISC License](LICENSE).
